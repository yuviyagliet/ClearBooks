...
export function Input({ id, ...props}){
  return <input id={id} {...props} className={`w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-700/20 focus:border-teal-700 bg-white ${props.className||''}`} />
}
export function Select({ id, ...props}){
  return <select id={id} {...props} className={`w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-teal-700/20 ${props.className||''}`} />
}
export function Label({ children, htmlFor }){ return <label htmlFor={htmlFor} className="text-xs font-semibold text-gray-600 uppercase tracking-wide">{children}</label>}
