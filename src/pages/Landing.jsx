    ...
    <div className="min-h-screen bg-white">
      <header className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {user ? <Link to="/dashboard" className="text-sm font-medium border border-gray-200 rounded-full px-4 py-2 hover:bg-gray-50">Go to dashboard</Link>
          : <>
            <Link to="/login" className="text-sm font-medium px-3 py-2">Log in</Link>
            <Link to="/signup" className="text-sm font-semibold bg-teal-700 text-white rounded-full px-5 py-2.5 hover:bg-teal-800">Sign Up Free</Link>
          </>}
        </div>
      </header>
      ...
      <section className="max-w-6xl mx-auto px-6 pt-12 md:pt-20 pb-12 grid md:grid-cols-2 gap-10 items-center">
        <div>
          ...
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight leading-[1.05] mt-4">Track income, expenses, and invoices — without the accounting-software bloat.</h1>
          <p className="text-gray-500 mt-4 text-lg leading-relaxed">ClearBooks is the minimal ledger for freelancers. Log money, send clean invoices, and export for tax season in minutes.</p>
          ...
          <div className="flex flex-wrap gap-3 mt-6">
            <Link to={user?'/dashboard':'/signup'} className="bg-teal-700 text-white rounded-full px-7 py-3.5 text-sm font-semibold hover:bg-teal-800">{user?'Go to Dashboard':'Get Started — Sign Up Free'}</Link>
